...
<#compress>
    ...
    <div class="container-fluid">
        ...
        <form class="form-horizontal js-file-form"
              action="/apps/${app.id}/configitem/${config.id}" method="post"
              data-remote="true" data-method="put" data-before="ajaxBeforeSend"
              data-location="">
            ...
            <div class="form-group">
                ...
                <div class="col-md-1">
                    <select multiple size="3" name="tagId" class="form-control"
                        ...
                        <#list tagList as tag>
                            <option value="${tag.id}"
                                    <#if ((config.tagIdList)![])?seq_contains(tag.id)>selected</#if>>${tag.name}</option>
                        </#list>
                    </select>
                </div>
                ...
                <div class="col-md-1 text-right">
                    <button type="submit" class="btn btn-primary" title="保存 (Ctrl+S)">保存文件</button>
                </div>
            </div>
        </form>
